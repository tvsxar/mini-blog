...
function PostItem({ post }) {
  ...
  return (
    <Link to={`/post/${_id}`} className="group max-w-full grid grid-cols-1 md:grid-cols-[.9fr_1.1fr] gap-6 md:gap-10 cursor-pointer items-center">
      <div className="w-full h-56 sm:h-64 md:h-72 lg:h-80 xl:h-96 overflow-hidden rounded-xl">
      ...
      <div className="flex flex-col justify-center gap-2 md:gap-4 px-4 md:px-0 pb-4 md:pb-0">
        ...
        <p className="text-lg sm:text-xl text-gray-700 leading-relaxed line-clamp-3 md:line-clamp-4 lg:line-clamp-6 xl:line-clamp-7 overflow-hidden text-ellipsis">
            {summary} 
        </p>
      </div>
    </Link>
